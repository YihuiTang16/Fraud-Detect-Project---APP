...
st.set_page_config(page_title="Fraud Assessment", layout="wide")
...
# ── Page header ───────────────────────────────────────────────────────────────
st.title("Fraud Assessment")
st.caption("Review anonymized company financials and classify each firm as fraud or clean. Results are compared against the model after each submission.")

# ── Progress bar ──────────────────────────────────────────────────────────────
completed = len(st.session_state.game_results)
progress = completed / total_rounds
st.progress(progress, text=f"Case {min(idx + 1, total_rounds)} of {total_rounds}")
st.markdown("---")

# ── Assessment complete screen ────────────────────────────────────────────────
if idx >= total_rounds:
    st.success("Assessment complete. View detailed results on the Model Performance page.")
...
st.markdown("Company identity is anonymized. Classify based on the financial ratios below.")
...
if not st.session_state.revealed:
    ...
    with col_conf:
        confidence = st.slider("Confidence (%)", 50, 100, 70, step=5)
...
if st.session_state.revealed:
    ...
    r2.metric("Your Answer", "Fraud" if user_pred == 1 else "Not Fraud",
              delta="Correct" if human_correct else "Incorrect",
